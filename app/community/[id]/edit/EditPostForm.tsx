...
function parseContentToBlocks(content: string): Block[] {
  ...
  for (const part of parts) {
    ...
    if (imgMatch) {
      blocks.push({ id: newId(), type: "image", url: imgMatch[1] });
    } else if (vidMatch) {
      blocks.push({ id: newId(), type: "video", url: vidMatch[1] });
    } else {
      blocks.push({ id: newId(), type: "text", content: part });
    }
  }
  if (blocks.length === 0) blocks.push({ id: newId(), type: "text", content: "" });
  return blocks;
}

export function EditPostForm({ postId, categories, defaultValues }: {
  postId: string;
  categories: Category[];
  ...
  const [blocks, setBlocks] = useState<Block[]>(() => parseContentToBlocks(defaultValues.content));
  ...
  return (
    <form action={action} className="bg-white rounded-xl border border-[#d4d4d4] p-6 space-y-4">
      ...
      <div>
        ...
        <select id="categoryId" name="categoryId" required defaultValue={defaultValues.categoryId} className="w-full h-11 rounded-lg border border-border px-3 text-sm text-foreground bg-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors">
      ...
      <div>
        ...
        <input id="title" name="title" type="text" required defaultValue={defaultValues.title} className="w-full h-11 rounded-lg border border-border px-3 text-sm text-foreground bg-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors" />
        ...
        <div className="rounded-lg border border-border bg-white focus-within:border-primary focus-within:ring-1 focus-within:ring-primary transition-colors overflow-hidden">
              ...
              if (block.type === "image") {
                return (
                  <div key={block.id} className="relative mx-3 my-2 flex justify-center">
                    ...
                    <button type="button" onClick={() => removeBlock(block.id)} className="absolute top-2 right-2 w-7 h-7 rounded-full bg-black/50 hover:bg-black/70 text-white flex items-center justify-center transition-colors">
          ...
          <div className="flex items-center justify-between px-3 py-2 border-t border-border bg-[#f8fafc]">
            <label className={`inline-flex items-center gap-1.5 h-8 px-3 rounded-lg text-xs text-[#5F6B7C] hover:bg-gray-200 transition-colors cursor-pointer ${uploading ? "opacity-50 pointer-events-none" : ""}`}>
              ...
              <input type="file" accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm,video/quicktime" multiple onChange={handleFileSelect} className="hidden" disabled={uploading} />
            </label>
      ...
      <div className="flex justify-end gap-2">
        <a href={`/community/${postId}`} className="h-11 px-5 rounded-lg border border-[#d4d4d4] text-sm font-semibold text-[#5F6B7C] hover:bg-gray-50 transition-colors inline-flex items-center">취소</a>
